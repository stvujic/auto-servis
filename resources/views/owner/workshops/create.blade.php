...
@section('content')
    <h1 class="text-2xl font-bold mb-4">Dodaj servis</h1>

    <form method="POST" action="{{ route('owner.workshops.store') }}" class="bg-white rounded-lg shadow p-5 grid gap-4 md:grid-cols-2">
        @csrf
        ...
        <div>
            ...
            <input type="text" name="city" value="{{ old('city') }}" class="border p-2 w-full" required>
            @error('city') <div class="text-red-600 text-sm">{{ $message }}</div> @enderror
        </div>
        <div>
            <label class="block text-sm mb-1">Adresa</label>
            <input type="text" name="address" value="{{ old('address') }}" class="border p-2 w-full" required>
        ...
        <div class="md:col-span-2">
            ...
            <textarea name="description" rows="4" class="border p-2 w-full">{{ old('description') }}</textarea>
            @error('description') <div class="text-red-600 text-sm">{{ $message }}</div> @enderror
        </div>
        <div class="md:col-span-2">
            <button class="px-4 py-2 bg-blue-600 text-white rounded">Sačuvaj</button>
            <a href="{{ route('owner.workshops.index') }}" class="ml-2 text-gray-700 hover:underline">Otkaži</a>
        </div>
    </form>
@endsection
